  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        await start(id, content.title, content.thumbnail, content.type, content.duration);
      } catch {
        setLoadingMeta(false);
      }
    };
    ...
    return (
      <SafeAreaView style={s.centered}>
        <View style={s.loginPrompt}>
          ...
          />
          <Text style={s.loginPromptTitle}>
            Connectez-vous pour télecharger le contenu
          </Text>
          <TouchableOpacity
            style={s.loginBtn}
  ...
  return (
    <SafeAreaView style={s.root} edges={["top", "bottom"]}>
      ...
      <View style={s.body}>
        ...
        {/* État */}
        {(status === "idle" || status === "pending" || loadingMeta) &&
          !alreadyDone && (
            <>
...
const s = StyleSheet.create({
  ...
  cancelBtn: {
    ...
    borderWidth: 1,
    borderColor: "rgba(237,51,59,0.3)",
  },
  cancelText: {
    color: colors.error,
